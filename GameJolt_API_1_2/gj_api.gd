...
signal request_error(result, response_code)
...
func add_score(score, sort, guest=null, extra_data=null, table_id=null):
	...
	if guest:
		queue_cmd(COMMAND.scores_add, [null, null, guest, score, sort, extra_data, table_id])
	else:
		queue_cmd(COMMAND.scores_add, [_username, _user_token, null, score, sort, extra_data, table_id])
	
func fetch_score_rank(sort, table_id=null):
	# retrieves the rank of the nearest score
	queue_cmd(COMMAND.scores_fetch_rank, [sort, table_id])
...
func fetch_scores(limit=null, table_id=null, guest=null, better_than=null, worse_than=null):
	...
	if guest:
		queue_cmd(COMMAND.scores_fetch, [limit, table_id, null, null, guest, better_than, worse_than])
	else:
		queue_cmd(COMMAND.scores_fetch, [limit, table_id, _username, _user_token, null, better_than, worse_than])
	
func fetch_tables():
	# fetches the ids of all tables for this game
	queue_cmd(COMMAND.tables_fetch, [])


func open_session():
	# opens a game session for the cached user
	queue_cmd(COMMAND.session_open, [_username, _user_token])

func ping_session(status=null):
	# call at least every 120s to keep session opened, optionally status can be 'active' or 'idle'
	queue_cmd(COMMAND.session_ping, [_username, _user_token, status])
	
func check_session():
	# checks if there is an open session in this game for the cached user
	queue_cmd(COMMAND.session_check, [_username, _user_token])
	
func close_session():
	# closes the game session for the cached user
	queue_cmd(COMMAND.session_close, [_username, _user_token])
...
func fetch_trophy(achieved=null, trophy_id=null):
	...
	queue_cmd(COMMAND.trophy, [_username, _user_token, achieved, trophy_id])
	
func set_trophy_achieved(trophy_id):
	# gives the cached user the specified trophy
	queue_cmd(COMMAND.trophy_add, [_username, _user_token, trophy_id])
	
func remove_trophy_achieved(trophy_id):
	# removes teh specified trophy from the cached user
	queue_cmd(COMMAND.trophy_remove, [_username, _user_token, trophy_id])
...
func auth_user(username, user_token):
	...
	queue_cmd(COMMAND.user_auth, [_username, user_token])
	
func fetch_user(username=null, user_id=null):
	# returns a user's data - only username or user_id is required
	queue_cmd(COMMAND.user_fetch, [_username, user_id])


func fetch_friends():
	# returns all friends of the cached user
	queue_cmd(COMMAND.friends, [_username, _user_token])



# ex.: $GameJoltAPI.callback(this, '_my_callback').some_api_func(...)
# or as two subsequent calls (but setting the callback first)
# this function has no effect until the next queue_cmd() call is done
...
var queue = []
func queue_cmd(cmd, args):
	# using a queue means code does not check if currently a request is open
...
func _on_HTTPRequest_request_completed( result, response_code, headers, body ):
	if result != RESULT_SUCCESS:
		emit_signal('request_error', result, response_code)
